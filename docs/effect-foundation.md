...
| **E3b** | ⬜ **Scoped modifiers** | `ModifierScope` on contributions, `scoped_by` on keys, throw-on-missing-context, `diminishing`/`highest_only` stacking modes, the `danger` cap rule. | E3a | medium |
