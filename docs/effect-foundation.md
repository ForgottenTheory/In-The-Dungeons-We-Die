...
| **E3c-3** | ⬜ **Stateful conditions** | `targetHasStatus`, `selfHasStatus`, `resourceAbove`/`Below`, `equippedTag`, `hitHasLane`, `actionHasTag`. **Zero authored uses today** — they are for future content. Requires `TriggerRuleEngine.Evaluate` to stop being a static pure function of the event and carry world state. | E3c | medium |
