...
**A *context* supplies a set per dimension, not a single value** (corrected in E3c-2). One swing
is `melee` and `attack` and `light` simultaneously, so `ModifierContext.Matches` is set
membership rather than equality. Equality would have silently dropped every `move_tag` modifier
on any move carrying more than one tag — which is every move.
...
| **E1** | ✅ **DONE — the Hit pipeline** | `Hit`/`Packet`/`DamageLanes`/`DamageAspects`, the ordered stage list, `HitLog`, diminishing armour, the resistance pipeline, enemy vulnerability, **Perfect Block**. `CombatCalculator` is now a thin façade over `HitPipeline`; `AttackProfile` bridges as a single packet (D-18). 22 new tests, 4 re-derived. | E0 | medium |
| **E2** | ✅ **DONE — lifecycle split, then statuses** | **E2a:** telegraph and windup are separate scheduler states; `ActionInFlight` unifies both sides; `Interrupt()` cuts an action and tags which phase it cut. Landed green with **zero** existing tests changed — total time-to-impact is identical. **E2b:** `StatusDefinition` + `StatusController` + **27 statuses** (14 core + 13 previously-dangling authored ids) + Resolve, shared control immunity and per-encounter escalation. `ContentValidator` now proves every authored status id resolves. 26 new tests. | E1 | high |
| **E3a** | ✅ **DONE — rule engine upgrade** | `effects[]` (one chance roll, N effects; the legacy single `effect` stays valid so no content migrated), `EffectTarget` selectors, `EffectContext` + the full proc-safety model: depth budget, once-per-chain, per-target ICD, `CanTrigger`, and the 64-effect fuse. Validator rejects proc depth above the default outside Anomalous content. 13 new tests, including the brief's fusion chain terminating. | E1, E2 | medium |
| **E3b** | ✅ **DONE — scoped modifiers** | `ModifierScope` + `ModifierContext`, eight closed dimensions, `scoped_by`/`danger` on keys, `diminishing`/`highest_only` stacking. All three §4.2.2 guards live: resolving a scoped key without its dimension **throws**, a mismatched scope is rejected at `Add`, and a `danger` key without a `max` fails validation. `Resolve` takes a context and **has no defaulting overload** — the explicitness is the mechanism. Registry: the six `profession.*` keys are `scoped_by: profession`, `combat.damage.flat` by `move_tag`, preservation and double-output converted to `diminishing` per §4.4. 22 new tests including the §4.2.2 pinning sweep and the §4.2.2 worked trace. | E3a | medium |
| **E3c** | ✅ **DONE — effect handlers** | Six combat handlers (`damage`, `areaDamage`, `heal`, `applyStatus`, `grantResource`, `interrupt`), `EffectTargetResolver` turning E3a's selectors into combatants, and **the gauge runtime** — `GaugePool`/`GaugeController` — without which `grantResource` had nothing to fill. **Galvanic's Charge accumulates.** `EffectContext` now propagates through `CombatEncounter.Publish` and `StatusController.Apply`, so a proc's proc is depth 2 and stops. Ailment ticks publish `canTrigger: false` (proc-safety rule 4), which only started mattering once handlers existed. 15 new tests. | E3b | medium |
| **E3c-2** | ✅ **DONE — the modifier read path** | `CombatantModifiers` assembles build statics + status `while_active` + gauge bands + timed grants into one `ModifierSet`; `TimedModifiers` stores `grantModifier`'s duration-limited grants. **Nothing in combat read a modifier before this**, so all four contributors were authored and inert. The pipeline now resolves `combat.damage.mult` (a new INCREASED stage), `crit.chance`, `crit.mult`, `armor`, `block.mult` and `damage_taken.mult`; the scheduler resolves `windup.mult`, which is what **Chill** is. First real `ModifierContext`, built from the hit. 11 new tests. | E3c | medium |
| **E3c-3** | ✅ **DONE — stateful conditions** | `IConditionWorld` (four questions, no entity graph) + `CombatConditionWorld`. New kinds: `targetHasStatus`, `selfHasStatus`, `resourceAbove`/`Below`, `equippedTag`, `hitHasLane`; `gaugeAtLeast` gains a gauge name. `Evaluate` keeps its static form with an optional world, and a condition the engine cannot answer is recorded in `UnevaluatedConditions` rather than returning false — the condition half of `Unhandled`. **`actionHasTag` deliberately not added**: it would be a synonym for `hasTag`, and D-11's rule is that a derived tag answers a real gap while a new condition kind does not. 12 new tests. | E3c-2 | medium |
| **E4** | ✅ **DONE — Moves** | `MoveDefinition` (timing + costs + packets + `EffectSpec` riders — one shape, Attack vs Spell is data), `ActionTiming`/`ActionCost` (the shared Action vocabulary), `MovesetBuilder` → `ResolvedMove` with provenance, `MoveModifier` (match + 11 ops in a **fixed application order**, proved order-independent), **`AttackProfile` and `AbilityDefinition` deleted** — weapons grant moves, enemies run weighted AI rules over the same `ConditionSpec` vocabulary, both allowlists deleted, `status.recalled_move` live (the Mnemonic loop closes end-to-end). Move riders route through the same `IEffectSink`/handler registry as rule effects; `triggerMove` executes at depth+1 with nesting refused statically. 9 shipped moves incl. the three §2.2 exemplars. 30 new tests; **the trace-read pass caught two engine bugs** (recalled moves losing the caster's modifiers; triggered moves not advancing chain depth). | E3 | **high** |
...
| **D-03** | Resistance lanes: `physical magic heat cold charge toxin corrosion decay` (8), **`kinetic` aspect (renamed from `arcane`, D44) has no lane and is unresistable**. | ✅ **DECIDED — adopted.** **(a)** Kinetic is unresistable, and the "best-in-slot" risk is closed **structurally**: kinetic accepts flat adds and *global* increases only — never a lane-specific `increased`, never a `more`/`less` multiplier, never conditional damage. It scales linearly while every other lane goes multiplicative, so it is strongest at low investment and weakest at high. Validator-enforced. **(b)** `decay` lane is **reserved in the vocabulary at E1, content authored later**. `damage-and-defense.md` §2.5.1, §4.1 |
...
| **D-27** | E1 pre-flight corrections. | ✅ **DECIDED — two fixes found while briefing E1.** **(a) `ArmourK = 1`, not 5.** The formula shape was borrowed from PoE along with its constant, but PoE armour is in the thousands and ours is single-digit — K=5 took iron armour from 89% to 18% against a light hit, gutting it rather than reshaping it. K=1 keeps the attrition-vs-spike shape and removes the `max(1,…)` cliff. Recalibrate in C2. **(b) The telegraph/windup split moves from E1 to E2.** `moves.md` §2.3 had it in E1, contradicting the D-18 reasoning that rejected two high-risk rewrites in one slice. Nothing in E1 needs it; Stun in E2 is the first thing that does. `damage-and-defense.md` §5.4, `moves.md` §2.3 |

---

## 12.1 Decision log — 27 settled
