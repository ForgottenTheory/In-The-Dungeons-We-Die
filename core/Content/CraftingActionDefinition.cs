...
/// <summary>
/// How a reagent releases what it carries (docs/emergent-item-system.md §7.3). A crafting action
...
/// <summary>One property the crafting action opens, and how fast it moves along that channel (§7.2).</summary>
public sealed class AffectedQuality
{
...
/// <summary>
/// How much each slot contributes to the resulting material strength (§6.1). Weights sum to 1.0 so
/// material strength stays a weighted mean and can never be gamed by dumping quantity into a slot.
/// </summary>
...
/// <summary>What a substrate must be before the crafting action will run on it (§7.2, §8.7 step 1).</summary>
public sealed class CraftingActionRequirements
{
    ...
    /// <summary>Minimum level in the crafting action's profession. Ignored when the crafting action is ungated.</summary>
    [JsonPropertyName("profession_level")]
    public int ProfessionLevel { get; init; }
}

/// <summary>
/// The tag edits a crafting action asserts on its result — the highest-priority source in the
...
/// </summary>
public sealed class CraftingActionTagEffects
{
...
/// <summary>
/// A crafting crafting action — the authored data that decides <i>which</i> properties react and how
/// violently (docs/emergent-item-system.md §7). CraftingActions are the only content the emergent
...
/// different channels are open. Choosing the crafting action is a first-class player decision.
///
/// <para>There are no recipes. A crafting action never references an item id.</para>
/// </summary>
public sealed class CraftingActionDefinition : IDefinition
{
    public string Id { get; init; } = string.Empty;
    public string Name { get; init; } = string.Empty;

    /// <summary>Gating profession id, or empty for an ungated crafting action (e.g. Grind).</summary>
    public string Profession { get; init; } = string.Empty;

    /// <summary>
    /// How violent the transformation is (0–1). Multiplies the workability charged for a given
    ...
    /// <summary>The properties this crafting action opens. Everything else is off-channel (§8.3).</summary>
    [JsonPropertyName("affected_qualities")]
    public IReadOnlyList<AffectedQuality> AffectedQualities { get; init; } = Array.Empty<AffectedQuality>();

    /// <summary>Rate at which essence transfers along this crafting action (§8.4). Consumed from P3.</summary>
    [JsonPropertyName("essence_rate")]
    public double EssenceRate { get; init; }

    public CraftingActionRequirements Requires { get; init; } = new();

    [JsonPropertyName("tag_effects")]
    public CraftingActionTagEffects TagEffects { get; init; } = new();

    /// <summary>True when no profession gates this crafting action.</summary>
    ...
    public double TransferRateFor(string property)
    {
        foreach (var entry in AffectedQualities)
        {
    ...
    /// <summary>True if <paramref name="property"/> is opened by this crafting action.</summary>
    public bool Affects(string property) => TransferRateFor(property) > 0.0;
}
