...
/// every combination of substrate, reagent and process produces a result, always. There is no
...
/// </summary>
public static class ReactionAlgebra
{
    /// <summary>
    /// Applies one reagent to a substrate under a process — steps 2–6 of the §8.7 pipeline:
    ...
    /// <param name="process">Decides which properties react, and how violently.</param>
    /// <param name="properties">The property registry — roles, opposition, floors.</param>
    /// <param name="substrateIntegrity">Remaining transformation budget (§6.2).</param>
    ...
    public static ReactionStepResult ApplyReagent(
        ...
        double catalyst = ReactionTuning.NoCatalyst)
    {
        ...
        var coefficients = ReactionCoefficients.For(
            substrate, reagent, process, substrateIntegrity, qualityMultiplier, catalyst);
        ...
        foreach (var key in Union(substrate, reagent))
        {
            ...
            var rate = process.ChannelRate(key);
            if (rate > 0.0)
            {
                var after = Converge(before, reagent.Get(key), rate, coefficients.Product);
                Record(state, changes, key, before, after, PropertyChangeKind.Channel);
                continue;
            }
            ...
            {
                var after = before * (1.0 - ReactionTuning.ReactiveDilutionRate);
            ...
            else
            {
                ...
                var after = before + (target - before) * ReactionTuning.StructuralBlendRate;
        ...
        return new ReactionStepResult(new PropertySet(state), coefficients, strain, changes);
    }
    ...
    {
        var k = Math.Clamp(rate * coefficientProduct, 0.0, ReactionTuning.MaxConvergence);
        ...
        List<PropertyChange> changes)
    {
        ...
        foreach (var key in state.Keys.ToList())
        {
            ...
            var annihilated = Math.Min(a, b) * ReactionTuning.AnnihilationRate;
            if (annihilated <= 0.0)
                continue;
        ...
        List<PropertyChange> changes)
    {
        foreach (var key in state.Keys.ToList())
        {
            ...
            var floor = definition?.Floor ?? ReactionTuning.DefaultFloor;
            if (value >= floor)
                continue;
    ...
    private static void Set(Dictionary<string, double> state, string key, double value) =>
        state[key] = Math.Clamp(value, ReactionTuning.MinPropertyValue, ReactionTuning.MaxPropertyValue);
}
