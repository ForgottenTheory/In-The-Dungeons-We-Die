...
public enum CraftFailure
{
    ...
    /// <summary>The substrate lacks a tag the crafting action requires (§7.2 <c>substrate_tags</c>).</summary>
    SubstrateRejected,
...
/// </summary>
/// <param name="CraftingActionId">Which crafting action — the choice that decides what reacts at all.</param>
...
///
/// <para>This exists because workability 0 destroys the material, and that rule is only fair if
...
public sealed record CraftPreview(
    ...
    int ProjectedMaterialStrength,
    string ProjectedName,
    bool WouldBeFirstDiscovery,
    ReactionLog Preview,
    MaterialState? Projected = null,
    IReadOnlyList<TransformationStepResult>? Steps = null)
{
    ...
    public bool WarnsOfDestruction => CanCraft && Workability.IsCertainDestruction;

    /// <summary>Destruction is possible; show the percentage (§6.2c).</summary>
    public bool WarnsOfRisk => CanCraft && Workability.IsAtRisk;
    ...
    public IReadOnlyList<TransformationStepResult> StepResults => Steps ?? Array.Empty<TransformationStepResult>();

    public static CraftPreview Failed(CraftFailure failure) => new(
        failure,
        new WorkabilityProjection(0, 0, 0, 0),
        ProjectedMaterialStrength: 0,
...
/// <param name="WasDestroyed">Workability reached zero (§6.2c). Byproducts are the consolation.</param>
