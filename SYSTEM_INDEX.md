...
- **Enemies use the same system**: `ActorDefinition.Moves` + `AiRuleSpec[]` (weighted rules over the shared condition vocabulary; empty = uniform). `CombatEncounter.ChooseMove` is seeded-deterministic. **`Combatant.Moveset`** replaced `AbilityIds`; `Combatant.Ai` carries the profile.
...
`species/`(3) `classes/`(15 Bases) `prefixes/`(25) `suffixes/`(50; 10 expressed) `professions/`(3) `profession_actions/`(3) `materials/`(~474 defs across 8 category array files incl. byproducts; `family:value` tags) `properties/`(21 `PropertyDefinition`s) `processes/`(7) `byproducts/`(4) `name_grammar/`(44 words) `crafting_interactions/`(1, legacy shim) `modifier_keys/`(51) `name_formats/`(9) **`moves/`(9 — weapon moves, the three spec exemplars, Recall, enemy ports)** `move_modifiers/`(0, mechanism landed E4; E5 populates) `actors/`(2, moves + optional AI profiles) `consumables/`(1) `equipment/`(4, resistances **keyed by lane**) `realms/`(1) **`statuses/`(28 — 14 core + all 14 previously-dangling authored ids, `recalled_move` included)**. Each folder auto-loads into a `DataStore<T>` in `GameRoot._Ready` (materials via array files, everything else one-object-per-file).
