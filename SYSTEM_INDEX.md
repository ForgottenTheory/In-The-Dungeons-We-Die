...
- **`TriggerRule`/`ConditionSpec`/`EffectSpec`/`RuleVocabulary`** + **`TriggerRuleEngine`** — declarative hooks: 17 condition kinds, 12 effect kinds, cooldowns, seeded chance. `IEffectHandler` is registered by the system owning the behaviour; unhandled effects are recorded, not dropped.
...
- **`CombatEffectHandlers` / `EffectTargetResolver`** (`core/Combat/`, E3c) — the seven effect kinds combat owns: `damage`, `areaDamage`, `heal`, `applyStatus`, `grantResource`, `grantModifier`, `interrupt`, registered via `engine.RegisterCombatHandlers(encounter, rng)`. The resolver turns an `EffectTarget` into combatants and filters the dead. Effect damage does **not** run the hit pipeline — "deal 12" is a flat number, not an attack, so it takes no armour, crit or avoidance. Everything else (`grantModifier`, `spawnEntity`, `grantItem`, `reposition`, `revealInfo`) is still `Unhandled` by design.
