...
`IDefinition`, `DataStore<T>` (load/lookup/dup-id validation, enum+case-insensitive JSON; `LoadDocuments` auto-detects single-object vs array files), `MaterialDefinition` (implements `IItemDefinition`; flat `Dictionary<string,double> Properties` on a 0–100 scale; `family:value` tags), `MaterialProperty` (name→value pair, used only for crafting-outcome reporting), `DuplicateDefinitionException`. **Emergent item system P0** (`docs/emergent-item-system.md`): `PropertyDefinition`/`PropertyRole`/`ResistContributor` (property registry, loaded from `game/data/properties/`), `ResistanceCalculator` (derives resistance from `resisted_by`, authored `*_resistance` = override), `TagFamilies`/`TagFamily`/`TagCardinality` (the `family:value` tag namespace). `ContentValidator` (static; cross-reference/well-formedness checks over the loaded stores, incl. material property range 0–100 + known-name validation, and tag-family cardinality) → `ContentProblem` list; `ContentValidationException`. **Connects:** `GameRoot._Ready` calls `ContentValidator.Validate(...)` after loading all stores (`ValidateContentOrThrow`), failing loudly on any problem; also loads `DataStore<PropertyDefinition>`.
