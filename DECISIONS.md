...
### D39 — Realm Preparation: the loadout is a plan, not a second inventory
*(Adopted 2026-08-18, Phase 7. Model: `core/Realms/RunLoadout.cs`, `core/Realms/LoadoutCheck.cs`; briefing: `core/Presentation/RealmBriefing.cs`, `core/Presentation/RealmFieldwork.cs`; screen: `game/ui/RealmPreparationPanel.cs`.)*

**The Hideout and the run were both built and nothing joined them.** Entering a Realm was a list of 164 `Enter <Realm>` buttons with no way to see what you were walking into or decide what you took. The preparation screen is the bridge the loop diagram has carried as `[PLANNED]` since the first commit (GDD §2, §11.7).

**A loadout stores where you are going and what you pack — and nothing else.** The obvious shape is a record of chosen equipment, and it is wrong: `Equipment` already *is* the gear half of a loadout, it already persists, and combat already resolves from it. A second copy means two answers to "what is the player wearing", and the day they disagree the bug is invisible. So the screen edits the real equipment through the normal equip path, and `RunLoadout` holds only the destination and the pack. **Save v10 is two fields.**

**Packing closed a real hole rather than adding a feature.** `CombatUseConsumable` reads the *active* bag, which inside a Realm is the run inventory — and `EnterRealm` created it empty. **A Healing Salve in the Stash was unreachable during a run.** Packing is the missing verb: the supplies transfer at entry, which also means they are unsecured from that moment and obey the extraction risk model with no second code path, exactly as gold does.

**The pack is a declaration, not a reservation, and it survives the run.** Supplies stay banked until entry, so a plan the player forgot about holds nothing hostage; and entering does *not* empty it, because a loadout you rebuild before every expedition is the chore the screen exists to remove. Outrunning the shelf **clamps** — take the two that are left — rather than failing, so spending a salve at the bench cannot silently break the next run.

**Every gear problem is a warning; only having nowhere to go blocks entry.** GDD §13.1 promises a player can never become permanently stuck, and a preparation screen that refuses to open the door is the easiest way to break that promise by accident. Naked, unarmed and carrying unreadable gear, the player may still walk in — with the starter kit one button away. `LoadoutCheckTests.NoAmountOfMissingGearEverStopsThePlayerEntering` is the fence. The starter kit reads the **Stash** too: an unequipped sword is not stuck, it is a click away.

**The briefing is a redaction, not a second knowledge system.** `RealmBriefing` lives in `Dungeons.Presentation` for the reason `AssayLens` does — it redacts a reading, it never changes one. Every gate goes through `RealmKnowledgeLevels.Reveals`; a second threshold table is how the screen and the in-run intel would start disagreeing about what the player earned. The hidden-node rule moved onto `RealmLocationDefinition.IsVisibleAt`, so travel, the map and the briefing cannot diverge.

**Two things the briefing found in existing content.**

1. **A negative resistance is a real weakness and the in-run intel drops it.** `KnowledgeIntel` lists only lanes above zero, so the fact that a plant burns never reached the player. The briefing splits on the sign: *hit it with* (damage types), *burns to* (negative lanes), *shrugs off* (positive lanes).
2. **A hidden exit is revealed by HiddenRoutes, not ExtractionRoutes.** The Split Trunk is both. Finding a node reveals what it is — the same rule the run uses, where standing on it lets you extract regardless. The first version of the test asserted otherwise and was wrong.

**Profession tools are absent on purpose.** The brief asked for them; they do not exist. Tool slots, tool forms and the yield pipeline that would read them are **E6**, and the Agility course's `CourseBonusKeys` are still unread. A tool slot with nothing behind it is a player-facing surface whose mechanic does not resolve, which rule 7 forbids. The Tools panel ships as **field readiness** instead — which trades the Realm asks for and how far your professions have come, via `RealmFieldwork`. It is real, it is useful, and it is where tool slots belong when E6 lands.

**Consequences:** save **v10** (a v9 save loads with no loadout — no migration); the Realm tab is now two screens that swap; `RebuildRealmControls` lost its out-of-realm branch; `GameRoot.DescribeInsight` moved to `Dungeons.Presentation.PreparationText` so the log and the screen share one wording; `EnemyRanks` names the `elite`/`boss` tags that three readers were spelling by hand. **Nothing here is balanced** — it shows numbers, it does not set them.

### D40 — Every progression track earns its keep, and none of them merge
*(Adopted 2026-08-18, Phase 8. Mastery: `core/Professions/Mastery*.cs`, `game/data/mastery/`; character: `core/Characters/CharacterProgress.cs`, `CharacterLeveling.cs`; knowledge: `core/Realms/RealmInsight.cs`. Fence: `tests/Progression/ProgressionEcosystemTests.cs`.)*

**Three tracks were tracked and not read.** Mastery incremented behind four hardcoded constants with no preservation, no doubling and no unlocks. Realm Knowledge had six of GDD §11.4's seven items and was missing the only one that grants an *option*. Character XP had the whole growth system — `AttributeGrowth`, the 4.0-point budget, `ResolvedBuild.GrowthAt` — and **no source**, so the character was permanently level 1 on a uniform-5 baseline. This pass connects all three without touching a balance curve.

**Mastery's numbers became content, and mastery levels stayed linear on purpose.** `game/data/mastery/` holds one shared ladder — six `MasteryBenefitKind` rungs, each with an unlock level, a per-level rate and a cap — rather than a mastery block on each of the 659 actions, which would be 659 places for a balance pass to miss one. **Mastery level is mastery points, one per completion, to a ceiling of 99.** A bending curve was the obvious move and it is wrong for this pass: it would silently reprice every action while claiming to be an integration. Linear means today's per-point numbers *are* today's per-level numbers, and the balance pass changes one method plus one JSON file.

**Preservation and doubling are unlocks, not creeping percentages.** Both carry an `unlock_level` and are worth exactly nothing below it, so mastering an action makes things *start happening* at 20 and 40 rather than adding a fourth number that has always been there. `ActionOutcome` reports both, and the log says so out loud — a benefit the player never sees fire is a benefit they do not believe in, which is most of how mastery came to feel inert in the first place.

**The action-specific unlock is an offer, not a bonus.** `ProfessionOpportunityDefinition.RequiredMasteryLevel` gates four of the game's highest-risk opportunities — The Reliquary, The Strongbox, the storm-charged bull, the unstable ember pocket. Below the gate they are **not rolled at all**, the same structural trick the active/passive seam uses: "a novice cannot find this" is a fact about the code rather than a very small probability. Deep mastery buys a different list of things that can happen.

**A validator rule caught the shipped table on its first run,** which is the argument for writing rules before data. Capping mastery at 99 means the old per-point rates never quite reach their old ceilings, so `mastery.interval` promising 0.5 while delivering 0.495 was a promise the ladder could not keep. Caps are now exactly what mastery 99 buys. **One number genuinely moved:** opportunity risk reduction, whose old cap of 0.5 needed 250 completions of one action — reachable for an uncapped counter, impossible for a 99-level track. Capped at what 99 levels buy (0.198) rather than raising the rate, because raising it would be a mid-track buff.

**Realm Knowledge was bracketed, never rescaled.** D38's five thresholds are exactly where it left them; Phase 8 added a rung below and a rung above. **`CommonResources`** (12) is what a place is *made of* — walked out of the node and creature loot tables with `LootReachability.ItemsReachableFrom`, so zero new content — and it exists because a first expedition that teaches the player nothing is a bad first expedition. **`DeepEntry`** (900) is GDD §11.4's *"unlock portal targeting"*, the only item on that list that hands over an option: begin a run at a deeper door. Last on the ladder because you cannot aim at a door you have not found, and priced honestly — starting at depth 2 skips depth 1's fights, its loot **and** its knowledge.

**Character XP comes from Realms and nowhere else — this is the load-bearing decision.** Defeating something pays by its resolved health and rank; extracting pays a flat award and **dying pays nothing**, which is the extraction decision applied to progression itself. Professions, crafting and discoveries award none. The alternative — everything feeds one XP pool — is the universal power level GDD §4 exists to prevent: fishing would raise combat attributes and every other track would become a rounding error. `ProgressionEcosystemTests.ProfessionWorkAwardsNoCharacterXp` holds it, and it fails at *compile* time if `ActionOutcome` ever grows the field.

**Levelling raises the ceiling and never refills what is under it.** `RebuildCharacter` composes a fresh `Character`, and a fresh one starts full — so levelling mid-run would have been a free heal at exactly the wrong moment for the extraction decision. Pools now carry across, clamped to the new maxima. Loading a save is the one deliberate exception: that is a rest.

**The fence is the real deliverable.** `ProgressionEcosystemTests` names every track and asserts something reads it, in the spirit of `NoProfessionIsADeadEnd`. Each of the three gaps this pass closed was found by *reading*, months late. Form/schematic acquisition is **exempt by name** with D29.2/M6 written beside it — `material.schematic_fragment` drops from eight tables and is inert, and binding it to forms is a balance and soft-lock decision (which forms does a fresh player know?), not an integration.

**Consequences:** save **v11** (a v10 save loads at level 1 — where every existing character already is); a new `mastery/` content type with nine validation rules and a failing-content test each; `ProfessionTuning` keeps the rounding and the floor and gave up the magnitudes; `LootRarities.FromTags` lifted out of `LootResolver` so the briefing and the drop roll read rarity the same way. **Nothing here is balanced** — the character curve, the XP-per-health rate and both new thresholds are first passes nobody has played.

### D41 — Idle and automation: one execute path, one combat resolver, and a summary you can read
*(Adopted 2026-08-18, Phase 10. Benefits: `core/Professions/ProfessionBenefits.cs`, `ProfessionSynergies.cs`, `game/data/synergies/`; auto-repeat: `core/Professions/PassiveProfessionRunner.cs`; the return: `core/Professions/AwayProgress.cs`, `core/Presentation/AwayReadout.cs`, `game/ui/AwaySummaryPanel.cs`; auto-combat: `core/Combat/AutoCombatPilot.cs`, `AutoCombatProfileDefinition.cs`, `game/data/auto_combat/`.)*

**The offline payout already worked; three things around it did not.** Levelling has never required staying online — `OfflineProgressCalculator` has looped the shared `ProfessionSystem.Execute` since P4. What was missing was everything that makes idle play a *loop*: bonuses that reward breadth, a selection that survives running out of ore, a return worth reading, and any way at all to fight while automated.

**One benefit seam, now three sources.** `ProfessionBenefits` folds the mastery ladder and the new synergy table into the single question the execution path asks — *what is this benefit worth, right now, for this action?* Because `ActionResolver` and `ProfessionSystem` already asked exactly once per benefit, **cross-profession and global bonuses arrived without a line of change in either file**. E6's worn tools are a third field here and nothing downstream. `MasteryBenefitKind` → **`ProfessionBenefitKind`**: the six quantities are no longer mastery's alone, and a name that claims a source it does not have is the naming mistake CLAUDE.md rule 8 is about. Member names are persistent identifiers and did not move.

**One content type covers both hooks, because they are one statement.** A `ProfessionSynergyDefinition` says "this much progress buys this much benefit"; a cross-profession row names the profession whose level pays, and a **global row names none and reads the player's total level across the roster**. Two types would have been two validators, two loaders and two ways to author the same number. The formula is the mastery ladder's — `unlock_level`, `per_level`, `max` — so a balance pass holds one rule in its head, not two. **Source and target must differ**: a profession paying for its own level is a mastery rung with extra steps, and a self-amplifying one, so the validator refuses it.

**Every shipped synergy follows a chain the professions already have.** Mining pays Smithing because ore actually walks from one to the other (docs/professions.md §5). A synergy between two professions that never touch is a number pretending to be a relationship, and the player would learn to read the table instead of the game.

**Auto-repeat: the selection is standing, not per-attempt.** A stalled action used to be forgotten, so "leave it running and come back" ended the first time a chest of ore ran dry — silently, and precisely on the sessions idle progress is for. `PassiveProfessionRunner` now holds `SelectedActionId` through a stall, waits, and resumes by itself when the materials return. **Temporary problems wait; permanent ones refuse** — missing inputs is a state the world can leave on its own, so selecting an action you cannot afford yet is a legal standing choice, while a level gate is still a refusal. Only Stop clears it. **No save change**: `PassiveActionId` now carries the selection rather than the running action, which is the same field meaning the same thing slightly more honestly.

**The return is a read-model, not more log lines.** For an idle game the return *is* the session's first beat. `AwayProgress` aggregates the absence — completions, crops lifted, items merged per id, XP, mastery, and **levels gained**, which the old three-line log never mentioned — and `AwayReadout` in `Dungeons.Presentation` is the one place its wording lives, so the console line and the panel cannot describe the same absence differently (D30). It **aggregates and never resolves**: every number came out of `Execute`. And it is honest about both ways a payout gets cut short, in different sentences, because "you were gone too long" and "you ran out of oak" are different problems and only one is worth acting on.

**Autosave on quit, guarded.** Offline time is measured from the save stamp, so without one, "leave it overnight" quietly means "remember to press Save first" and the night that gets forgotten looks like a bug. It refuses to write when no save file exists — a fresh game started for a look around must not overwrite a real one — and refuses inside a Realm, where unsecured loot is unsecured by design.

**Auto-combat is the player on the enemy AI machinery, and that is literal.** `AutoCombatPilot.Engage` puts the profile's `AiRuleSpec` rules onto `Combatant.Ai` and asks `CombatEncounter.ChooseMoveFor` — the method every enemy has always used. It then issues `UseMove` / `Block` / `Dodge`, the same commands a keyboard issues. **There is no second combat resolver and there must never be one:** the moment automated play has its own maths, passive and active are two balance models wearing one name.

**Its entire disadvantage is `reaction_ticks`, and the derivation is the point.** An agent whose hand is R ticks behind its eye must commit a stance R ticks before impact — and every tight window is measured from the moment the stance went up, so R ticks early is outside all of them. At R=8 it blocks (16-tick window) and dodges (10) reliably, and can never land a Perfect Block (4) or a Parry (3). A second constraint falls out for free: an attack arriving sooner than 2R after it appears cannot be answered at all, so fast and untelegraphed moves beat automation — which is why the small untelegraphed-only `evade` passive survived D-07. **No damage multiplier exists anywhere in the pilot**, and `AnAutomatedSwingHitsForExactlyWhatAManualOneDoes` is what fails the day one is added.

**The reaction floor is a validator rule, not a comment.** D-07 only holds while the latency is genuinely longer than the tight windows; a profile authored at 2 ticks would parry and quietly repeal the decision with nothing anywhere reporting it. `AutoCombatTuning.MinimumReactionTicks` is derived from the windows themselves, so retuning a window retunes the floor with it.
...
### D42 — Identities replace the property simulation (the crafting redesign)
*(2026-08-20, user-directed. Design of record: `docs/identity-foundation.md`. Implementation not started — the shipped code still runs the old model until the migration lands.)*

**The 21-property hidden simulation is replaced by named material identities.** A material stops being a map of 0–100 numbers pushed around by an algebra and becomes a small set of named, player-legible identities (Dense, Vital, Ember…, 24 in all — D44), each opening an **effect family** rather than granting one stat. Crafting becomes verbs over identities — reveal, isolate, transfer, combine, develop — across professions, still with **no recipe table anywhere**: D7's goal (emergence, a total function, no per-combination rules) is kept in full; only its mechanism is replaced.

**Why:** the old system produced genuine emergence that neither the player nor the developer could reason about without reading equations. The presentation layer (D30) was built to *translate* an illegible simulation; the redesign makes the simulation itself legible, which is strictly stronger. The replacement philosophy: *simple, understandable rules interacting to create deep emergent results* — "I did this to this material, which is why the item behaves this way."

**Rejected:** keeping the numeric layer under a better veneer (translation was already tried — the illegibility is in the model, not the wording); an authored recipe table (kills the emergence that is the system's entire reason to exist); making divergent materials per-unit instances again (D20 stands).

**What survives from the old system, deliberately:** preview parity (preview and commit are one computation), identical-results-stack (D20), names from final state never history, destruction pays byproducts, skill narrows variance, capacity-with-overflow (the essence capacity/strain idea, generalised), and the three-languages discipline (D30 — now with less to translate).

**Consequences:** the trait and essence layers are absorbed (D44); the genome/pressure model and all 44 property-keyed affixes are re-grounded on identities; the material library is re-authored (ids stable, stats block replaced); the emergent-id hash is re-inputted wholesale, which makes this a save re-identification event — the save stance (break vs migrate) is **an open decision that must be taken before Phase 1**. This decision also supersedes the crafting half of the "playtest before retuning" backlog. Migration runs in phases (foundation on paper → fences → coexisting core model → engine → item generation → library → professions → presentation → deletion), with `dotnet test` green throughout. Superseded-as-design banners sit on `crafting-overview.md`, `emergent-item-system.md`, `itemization.md`, `affixes.md`, `crafting.md`, `worked-examples.md`.

### D43 — Signature Profiles and the effect grammar
*(2026-08-20, user-directed, same package as D42. Full design: `docs/identity-foundation.md` §6–§9.)*

**Identities are the reliable floor; Signatures are the personality layer that makes two materials with the same identities behave differently.** Each material may carry an authored `SignatureProfile` — themes, favored triggers/behaviors/payloads, interaction biases, hidden tendencies, exclusions — all **weights and tendencies, never recipes**: Oak + Iron must not resolve to one predetermined result. The Signature Resolver reads the *exact* source materials, current identities, process/history, form, quality, catalysts and every profile, then generates and scores candidate effects. Oak-derived Vital, Willow-derived Vital and Bloodroot-derived Vital can therefore feel different while sharing an identity.

**A signature effect is a sentence — trigger × behavior × payload — and a generated Signature holds 1–N sentences** (simple = one; advanced/rare = several related ones, e.g. a cursed axe that poisons its wielder *and* raises Fishing). The grammar is a **generator over machinery that already resolves** — sentences compile to `TriggerRule`s, effect kinds, modifier keys, gauges and move ops. Triggers bind to shipped `GameEvents`; behaviors are a semi-closed verb set where each verb is one small registered assembler (the D16 bargain: new combinations are data, a new verb is one assembler); payloads are registry entries binding to live vocabulary. Four small effect-kind gaps were identified up front (`consumeStatus`, area status application, delayed effects, `cleanse`) plus one seam (`LootResolver` reading player luck).

**Themes are hidden scoring metadata only** — never player-facing, never a second identity system, never a new simulation layer. **Identity-family compatibility is the sensible default, not creative law:** procedural generation strongly favors family-coherent sentences by weight, and authored data may explicitly breach the defaults with no special-case engine code. The one fence that never bends is D30: nothing may be *registered* into the grammar without binding to machinery that resolves in play — weirdness is unrestricted, lying to the player is impossible. A reserved, currently-empty resolver stage is named for the future **Emergent Phenomena** system (rare rule-bending interactions); it is deliberately not designed yet.

**Rejected:** identity = effect 1:1 (collapses the possibility space the families exist to create); hard family gates (the first bizarre authored item would need engine special-casing — the exact failure mode this project refuses); a free-text effect DSL (unvalidatable; the registered-binding rule is what keeps tooltip and mechanics one thing). **This system absorbs the old P4 "Signature reactions" concept**, and the word *signature* now means only this layer — the SHA-256 stacking hash is renamed **fingerprint**.

### D44 — The identity roster and the naming rulings
*(2026-08-20, user-approved. Roster, boundaries and family sketches: `docs/identity-foundation.md` §3.)*

**24 identities in 7 clusters:** Dense, Hardened, Earthen · Keen, Balanced, Swift · Vital, Leeching, Thorned · Ember, Frost, Storm, Venomous, Corrosive, Serrated · Warded, Arcane, Resonant · Verdant, Radiant, Umbral, Blighted · **Charmed** (fortune/yield/opportunity — the extraction-economy axis nothing else covered, and the identity that gives crafted tools their point) · **Pure** (a meta-identity: it modifies crafting itself, not combat).

**Rulings:** **Kinetic is cut** — impact folds into Dense, repeated-action into Resonant/Swift; momentum stays on file for a future in which its mechanics exist. Stability moved Dense→Earthen (*Dense hits, Earthen refuses to move, Hardened shrugs it off*). **Accuracy is deleted** from Keen and Balanced — no attacker-accuracy stat exists, and a family list may not promise a mechanic the combat model doesn't resolve (D30 discipline; same reason movement/knockback claims were trimmed everywhere). Stamina economy → Swift, mana economy → Arcane. One primary home per mechanic, with deliberate build-changing crossovers. **The family sketches are possibility space, not player-facing evolution trees** — effect-family *access* progression is approved as a concept; **named identity evolution (Vital → Regenerative → …) is explicitly unresolved**, and player-facing tier numerals are rejected permanently.

**The essence layer is absorbed into identities:** fire→Ember, frost→Frost, storm→Storm, nature→Verdant, necrotic→Blighted, radiant→Radiant, abyssal→Umbral (the corrosion *lane* remains Corrosive's mechanical home). `essences.json` retires; the `essence` scoped-modifier dimension and the `essence:*` tags become the identity dimension in the same sweep. **D29.3 survives translated:** the richest identity-bearing reagents remain opportunity-gated and active-only. **Rejected:** keeping essence as a separate supernatural layer — two vocabularies for one concept, and the 7 essences were already proto-identities.

**Names:** the **`arcane` damage aspect is renamed `kinetic`** (user's suggestion — the word freed by the roster cut names exactly what the aspect is: raw unresistable force; D-03a semantics unchanged; swept with content in the same commit, including `spells_arcane.json`). The Arcane *identity* keeps the magic/mana meaning. The fortune identity is **Charmed** (Fated/Favored rejected — Favored collides with the profile's `favored_*` field family). **"Identity" stays** as both the design and player word; the old "identity = hash" usage dies with the old system. Transitional collisions (trait ids Keen/Venomous/Verdant/Blighted, the Resonant innate, the `resonance` property) die with the layers they live in — migration care, no permanent renames.

### D45 — Capacity, stability, condition, latents, and the material representation
*(2026-08-20, user-approved. Full design: `docs/identity-foundation.md` §10–§11; reality tests §12.)*

**Capacity is stable slots + risky overflow.** A material has a small authored **stable capacity** for distinct identities (usually 1–4). **Breadth and depth are separate axes by rule** — identity rank never consumes capacity, so the crafting question is always "which identities, how developed", never a points budget. Within capacity, crafting is predictable and controllable — the floor promise. Exceeding it is **advanced crafting, not an error**: the transfer succeeds and the material steps a visible ladder, **Stable → Unstable → Volatile** — reduced control, wilder signature generation where unusual outcomes score higher, fracture (identity-loss) risk, drawback sentences on minted items at the deep end, and **raised Emergent Phenomena odds** (the anomaly seam's first designed input). Expansion is rare and expensive (Pure's turf + authored catalysts); deliberate displacement/isolation are crafting verbs assigned in the Step 4 profession pass. **Rejected:** a weight budget (sum of ranks ≤ B — the old hidden arithmetic sneaking back); a hard wall (kills the expert gamble the user explicitly wants); depth ceilings per material (a knob nothing asked for yet).

**Latent identities ship.** A material may carry identities present-but-inactive (Oak holding latent Vital); latents don't occupy capacity, revealing one requires a free slot, and Assay's new job is detecting and reading potential rather than unredacting numbers. Gathering/analysis professions get discovery gameplay from one list on the definition. **Rejected:** always-visible identities — it deletes the "reveal" verb and Assay's reason to exist in the new model.

**Condition replaces the integrity float.** A four-step visible ladder — **Pristine → Worked → Strained → Fragile** — carries the old "push one more or commit now" tension (the deliberate rhyme with extraction) with zero hidden arithmetic. Identity-changing actions step it down (default one step; gentle work free; a restoration verb climbs it back — all provisional); at Fragile, ambitious work risks destruction, **which still pays byproducts**. **Rejected:** no budget at all (loses the commit tension on safe crafts); keeping a numeric budget (finer costs weren't worth surface arithmetic).

**The representation is eight facets, each readable in a sentence** — identities `{id, rank 1–4}` · latents · capacity · stability · condition · quality · merged profile · bounded provenance — against the old 21 floats plus five derived layers. Ranks are internal integers rendered as qualitative language (numerals rejected, D44). **The fingerprint** (stacking hash) covers sorted identities+ranks, capacity, stability, condition, quality bucket, quantized root composition and `form:`/`state:` tags; it deliberately excludes full crafting history (state is canonical — two paths to the same state stack) and profile content (the merged profile derives from roots, so roots cover it). **Profile merge** is a weighted union by contribution share — renormalized, trace-pruned, bounded, exclusions unioned — deterministic within a craft, order-sensitive across crafts because intermediate states are real inputs, which is exactly why process order matters. Deferred on purpose: rank accrual/transfer math (transformation-verbs design), item-side expression through forms (Phase 3), and every specific number.

**The Step 3 reality tests passed without new rules** (`identity-foundation.md` §12): plain iron gear stays plain; Dense Iron and the Oak→Herblore→Alchemy→Smithing chain work recipe-free; Dense Oakbound Iron stacks by fingerprint; the same material leans defensive on a buckler and aggressive on a longsword purely through form bias; and the overfill gamble is reachable and legible. Findings carried forward: the name grammar needs root-derived adjectives ("Oakbound"), carriers (capacity-1 extract vessels) are a content pattern, and the base-stat channel is the one remaining blocked computation — next design item.

### D46 — The base-stat channel: four visible physical stats
*(2026-08-20, user-approved. Full design: `docs/identity-foundation.md` §11.5.)*

**The mundane physical floor is four visible stats — Heft, Bite, Toughness, Give — 0–10 integers in an optional `base` block on the material.** They are player-facing gameplay numbers like item damage, not hidden simulation values. Heft is the heavy/light axis (impact and block up, speed down, stagger); Bite is edge-taking (cutting/piercing damage); Toughness is protection (armor, block strength); Give is flexibility (hafts, bow staves, grips, cloth — the flexibility-weapon axis, and the reason the set is four rather than three).

**The dividing line is a standing rule:** *if a physical quality only matters for special or magical behavior, it is an identity, not a base stat.* Elemental resistance is Warded/elemental-identity turf, crit is Keen, mana is Arcane — which is what holds the channel at four stats forever instead of regrowing property-space. **Identity work never moves base stats** (transferring Vital doesn't change iron's Bite); substance-changing verbs may (transformation-verbs design); workmanship is **quality**, which scales base delivery at fabrication. Emergent materials inherit base from their physical roots weighted by contribution — roots are already fingerprinted, so base adds nothing to the hash.

**Forms keep the mechanism that worked.** Slots, mass shares and tag gates survive; the old `stat_map` becomes **base reads** — the form declares which slot's base stat feeds which item stat (Longsword: Bite off the edge; Warspear: Give off the haft; a bow: Give off the stave). "No best material, only best placement" was never the stat map's fault — the 21 hidden inputs were. One scale constant maps base units to combat units, parity-pinned to the authored Iron Sword, as before.

**Consequences:** plain gear is viable on base alone — the mundane floor carries the early game and identities are the growth axis, not the entry fee; Focus and Ring become almost pure identity items (a plain oak focus is a stick — correct fiction); the material migration simplifies to authoring four visible integers instead of estimating 21 hidden floats, and Content Studio's material analysis becomes four-stat views. **Rejected:** three stats (loses the flexibility axis and with it bow/spear/whip physical differentiation); five-plus axes (drifts back toward property-space); grade/tier labels per role (violates the standing anti-tiering rule); keeping any hidden numeric layer (the thing D42 exists to delete).

### D47 — Ten transformation verbs, and the rank economy that interlocks the professions
*(2026-08-20, user-approved. Full design: `docs/transformation-verbs.md`.)*

**Verbs are code; actions are data.** The crafting operations of the identity system are a closed set of ten verbs, one executor each — **Process, Fuse, Reveal, Transfer, Develop, Extract, Displace, Refine, Restore, Expand** — and everything the player clicks is a crafting-action content entry (`verb + parameters + gates + costs + fiction name`). Smelt and Grind are Process actions; the forge's Alloy is Fuse wearing its fiction; Distill is Extract + Refine; Attune dies with the essence layer and Expand takes its niche. A new station's worth of actions is authoring, never engine work — the D16 bargain, applied a third time (effect kinds, behaviors, now verbs). Every verb specifies its facet effects and its condition class (gentle · identity-changing · ambitious), and preview parity is mandatory for all ten.

**The rank economy is the load-bearing rule.** Transfers from **raw** sources deliver rank 1; transfers from **prepared carriers** deliver the carrier's full rank (Extract preserves rank onto the carrier; Develop deepens it there by **feeding same-identity sources**). *Preparation = fidelity*: Distilled Oak Tincture (Vital r2) beats raw oak at the forge because Herblore and Alchemy touched the chain — the cross-profession ecosystem becomes economically real with zero mandatory recipes, and every identity-bearing material in the world gains a sink through Develop. Ranks never decay passively; loss is always a chosen risk (fracture on the overfill ladder, destruction at Fragile — byproducts paid as ever).

**Variance has exactly three homes, all previewed, all opted into:** signature generation at item time, the overfill ladder's wildness, and Fragile-plus-ambitious destruction rolls. Verbs themselves are deterministic at the floor. Quality is the control stat (tighter signature scoring, better base delivery, Develop efficiency), earned through the existing active-timing mechanic — and verbs award profession XP and mastery, closing the shipped gap where bench work trained nothing.

**Deliberately not verbs:** profile/theme manipulation (personality is shaped only by choosing sources); stabilize-in-place (the only exits from overfill are Extract, Displace, or Expand — capacity is a count, not a mood); un-fabrication (the door stays terminal). **Rejected:** per-station bespoke processes (the old model's coupling, recreated); all-transfers-preserve-rank (deletes preparation's payoff and with it the reason professions feed each other); input-free Develop (professions become self-sufficient and the ecosystem decays to twenty XP bars — the exact failure `ProfessionEcosystemTests` exists to prevent).

### D48 — Profession assignment: domains, not verb monopolies
*(2026-08-20, user-approved. Full matrix: `docs/transformation-verbs.md` §8.)*

**Two structural rules.** (1) **Professions own domains, not verbs** — the working professions (Smithing/Leatherworking/Tailoring/Fletching/Artifice) share the same core verb kit scoped to their material domain by substrate tags; nobody is "the Transfer profession". (2) **Solo-complete, chain-enhanced** — every offered verb works with raw inputs alone; other professions' products only raise fidelity, efficiency and odds through D47's rank economy. The brief's anti-busywork guard is now structural: no craft requires a second profession to be *possible* (deliberate exceptions: Expand, and Cooking's named dead-end until consumable forms).

**Three approved niches.** **Runecrafting is the one identity-scoped profession** — Transfer/Develop for magical identities (Arcane, Resonant, Warded, Radiant, Umbral) on *any* substrate, plus late Expand: the home of cross-substrate magical work that no domain profession covers. **Alchemy is depth** — universal Extract (late), Develop/Refine on carriers, Fuse for solutions, and co-home of Expand: the profession that concentrates, deepens, and pushes limits. **Mining and Forestry get bench Reveal in their own domains** (prospecting ore, reading timber grain) — gatherers gain a knowledge-flavored active verb where their expertise lives, alongside Herblore (flora) and Beast Lore (fauna). Also settled: **Salvaging's output is the Restore feedstock** (the repair economy's supplier); **Farming's niche is renewable identity sources** (seeds carrying latents, cultivation quality — supply-side, provisional, not the Develop verb); **Assay stays information-only** (detects latents, reads profiles/capacity/condition — gates legibility, never capability); Displace arrives late in every Transfer-capable profession's own scope rather than being anyone's toll; Thieving supplies exotics and rare catalysts; Agility and Cartography stay out of crafting.

**Rejected:** verb monopolies (each verb owned by one profession — makes multi-profession chains mandatory, the exact busywork failure the brief forbids); substrate-scoping Runecrafting (leaves cross-substrate magical work homeless); reveal-only-at-working-stations (keeps gatherers pure faucets and moves discovery away from the source). The ecosystem tests stay true with a reason behind them: processing professions eat each other's output because prepared inputs are mechanically better, not because a list says so.

### D49 — Save stance for the identity migration: progression survives, items reset
*(2026-08-20, user-approved. Closes the migration's gating question; see `docs/identity-foundation.md` §16.)*

**When the identity system replaces the property system, existing saves keep their progression and lose their items.** The redesign never touches the progression half of the save — `ProfessionSave` (levels, XP, per-action mastery), realm knowledge, character XP, learned moves, discoveries, Farming plots, the training course — so those sections load as-is under the new schema. The crafting-touched sections (stash stacks and instances, worn equipment, the emergent material and equipment registries) are **dropped**: their DTOs are removed, the serializer ignores the now-unknown fields in old files, `CurrentSchemaVersion` bumps, and the existing starter-kit rule (issue gear only when no weapon exists anywhere) re-equips the player on first load.

**Why:** item instances and emergent archetypes are frozen *property-shaped* snapshots, and the identity model re-derives none of it — there is no faithful mapping from 21 quantized floats to identities/ranks/profiles, only invented data wearing old names. Meanwhile the progression shapes are genuinely unchanged, so keeping them costs almost nothing and respects D14/D33's additive doctrine for the half of the save the redesign never touches. **Rejected:** a full wipe (needless loss of twenty professions of test progression); item conversion (fabricated provenance — the one thing this system must never do is lie about where an item came from).
