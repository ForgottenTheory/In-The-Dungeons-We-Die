...
- ✅ **Traits (P2/C1a)** — 16-trait library (`game/data/traits/`, `Category` per trait), `TraitResolver` birth→supersede→cap-3, displacement without refund, `id:tier` in the signature, `traits_created × 4` integrity cost.
- ✅ **Essence (P3/C1b)** — seven typed essences with anchors/opposition, additive §8.4 transfer, resonance×1.5 capacity → strain → effective instability, `Attune` (8th process), 38 materials with essence + 22 with resonance; essence in signature + save.
- ✅ **Fabrication (P5a+P5b/C2a+C2b)** — `FormTemplateDefinition` + `FabricationEngine`; terminal; derived `EquipmentDefinition` archetypes registered by signature and **persisted** (`EmergentEquipment`); 0–100→combat-unit calibration in `FabricationTuning` pinned by iron-sword parity; 3-slot Longsword (placement reorders trait dominance + §16.5 naming), Buckler, Vest; aperture-gated expression with dormancy; armour lane resistances from response properties; per-slot tag-filtered component UI.
- ⬜ Signature reactions (P4), consumable forms (P5c), codex/assay/rename (P6).
