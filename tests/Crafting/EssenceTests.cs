...
public class EssenceTests
{
    ...
    private static ProcessDefinition Forge(double essenceRate = 0.45) => new()
    {
        Id = "process.test_forge", Name = "Test Forge",
        Medium = TransferMedium.Thermal, Severity = 0.5, EssenceRate = essenceRate,
        Channel = new[] { new ChannelEntry { Property = "heat", Rate = 0.8 } },
    };

    private static ReactionCoefficients Coefficients(double acceptance = 0.5, double release = 0.8) =>
        new(acceptance, release, IntegrityFactor: 1.0, Catalyst: 1.0, QualityMultiplier: 1.0);
    ...
    /// strain the integrity cost feels; only the asymmetry survives. Authored one-sided here,
    ...
    public void OppositionAnnihilatesTheOverlap_EvenWhenAuthoredOneSided()
    {
        ...
        Assert.Equal(22.5, result.StrainReleased, 3);
    }
    ...
    public void StrainIsEssenceBeyondResonanceCapacity_AndFeedsEffectiveInstability()
    {
        ...
        Assert.Equal(12, EssenceTuning.Strain(Vector(("fire", 42)), 20), 3);
        Assert.Equal(0, EssenceTuning.Strain(Vector(("fire", 30)), 20), 3);

        // §6.2b: instability + strain + lost-integrity × 0.4.
        var calm = IntegrityCalculator.EffectiveInstability(10, 100);
        var strained = IntegrityCalculator.EffectiveInstability(10, 100, essenceStrain: 12);
    ...
    public void EssenceJoinsTheSignature_AndAnEmptyVectorChangesNothing()
    {
        ...
        var bare = new MaterialProfile(PropertySet.Empty.With("hardness", 60), 40, 70, lineage, string.Empty);
    ...
    public void InfuseStrainAttune_TheWholeLessonEndToEnd()
    {
        ...
        var infused = harness.Engine.Resolve(
        ...
        var profile = harness.Registry.All.Single(m => m.Id == infused.ResultItemId).Profile!;
        ...
        var attuned = harness.Engine.Resolve(
        ...
        var attunedProfile = harness.Registry.All.Single(m => m.Id == attuned.ResultItemId).Profile!;
        Assert.True(
    ...
    public void EssenceAndTraitsSurviveTheArchetypeRoundTrip()
    {
        ...
        var outcome = harness.Engine.Resolve(
            new CraftRequest("process.forge_infusion", "material.iron_ingot", new[] { "material.ember_core" }));
        Assert.True(outcome.Success);
        var original = harness.Registry.All.Single(m => m.Id == outcome.ResultItemId).Profile!;
        ...
        var restored = freshRegistry.All.Single(m => m.Id == outcome.ResultItemId).Profile!;
