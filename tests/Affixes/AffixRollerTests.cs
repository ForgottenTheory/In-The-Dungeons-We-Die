...
public class AffixRollerTests
{
    ...
    private static Genome Weapon(
        double hardness = 0, double mass = 0, double heat = 0, double flexibility = 0,
        int potency = 50, IReadOnlyDictionary<string, double>? essence = null) => new(
        "form.longsword",
    ...
    public void EligibilityIsAHardGate()
    {
        ...
        Assert.True(AffixRoller.IsEligible(brutal, Weapon(mass: 45), Array.Empty<string>()));
        Assert.False(AffixRoller.IsEligible(brutal, Weapon(mass: 20), Array.Empty<string>()));
    }
    ...
    public void WeightScalesWithPressure()
    {
        ...
        var low = AffixRoller.WeightOf(brutal, Weapon(mass: 30));
        var high = AffixRoller.WeightOf(brutal, Weapon(mass: 80));

        Assert.True(high > low, $"weight should scale with pressure ({low} → {high})");
    }
    ...
    public void TheTierCeilingFollowsTheGenome()
    {
        ...
        Assert.Equal(4, AffixRoller.TierFor(brutal, Weapon(mass: 35))!.Tier);
        Assert.Equal(2, AffixRoller.TierFor(brutal, Weapon(mass: 70))!.Tier);
        Assert.Equal(1, AffixRoller.TierFor(brutal, Weapon(mass: 90))!.Tier);
    }

    [Fact]
    public void PotencyPositionsTheRollInsideTheTier()
    {
        // §3.3's fourth lever: same tier, higher potency, higher value — no variance on innates.
        var keenEdge = Affixes.GetById("affix.innate_keen_edge");

        var weak = AffixRoller.Innates(Weapon(hardness: 60, potency: 10), new[] { keenEdge }).Single();
        var strong = AffixRoller.Innates(Weapon(hardness: 60, potency: 95), new[] { keenEdge }).Single();

        Assert.Equal(weak.Tier, strong.Tier);
        Assert.True(strong.Roll > weak.Roll, $"potency should raise the roll ({weak.Roll} → {strong.Roll})");
    }
    ...
    {
        var genome = Weapon(hardness: 90, mass: 90, flexibility: 90, potency: 80);

        var first = AffixRoller.Innates(genome, Affixes.GetAll());
        var second = AffixRoller.Innates(genome, Affixes.GetAll());
    ...
    public void TracePressureEarnsNoInnate() =>
        Assert.Empty(AffixRoller.Innates(Weapon(hardness: 10, mass: 10), Affixes.GetAll()));
    ...
    {
        var genome = Weapon(hardness: 70, mass: 60, potency: 60);

        var first = AffixRoller.Roll(genome, "prefix", Affixes.GetAll(), new SeededRandom(42));
        var second = AffixRoller.Roll(genome, "prefix", Affixes.GetAll(), new SeededRandom(42));
    ...
    {
        var genome = Weapon(hardness: 90, mass: 90, heat: 60, flexibility: 70, potency: 70);

        for (var seed = 0; seed < 50; seed++)
        {
            var rolled = AffixRoller.Roll(genome, "prefix", Affixes.GetAll(), new SeededRandom(seed));
            var families = rolled
    ...
    {
        // No heat pressure → Searing (burn on hit) can never appear, at any seed.
        var genome = Weapon(hardness: 90, mass: 90, potency: 70);
    ...
    /// across many rolls, and tier ceilings are never exceeded by any genome.</summary>
    [Fact]
    public void DistributionFollowsWeightsAndCeilingsHold()
    {
        var genome = Weapon(hardness: 88, mass: 40, potency: 60); // keen-favoured, brutal-light
        var counts = new Dictionary<string, int>(StringComparer.Ordinal);
        var rng = new SeededRandom(7);

        for (var i = 0; i < 20_000; i++)
        {
            foreach (var rolled in AffixRoller.Roll(genome, "prefix", Affixes.GetAll(), rng))
            {
                counts[rolled.AffixId] = counts.GetValueOrDefault(rolled.AffixId) + 1;

                var definition = Affixes.GetById(rolled.AffixId);
                var ceiling = AffixRoller.TierFor(definition, genome)!.Tier;
        ...
        // The genome shapes the pool: the heaviest eligible candidate by computed weight must
        // out-appear the lightest — expectation derived from WeightOf itself, never hardcoded.
        var eligible = Affixes.GetAll()
            .Where(d => d.Slot == "prefix" && AffixRoller.IsEligible(d, genome, Array.Empty<string>()))
            .Select(d => (d.Id, Weight: AffixRoller.WeightOf(d, genome)))
            .Where(x => x.Weight > 0)
            .OrderByDescending(x => x.Weight)
            .ToList();
        Assert.True(eligible.Count >= 2, "the test genome should support at least two prefixes");
        Assert.True(
    ...
    public void StatGrantsBecomeContributionsAndDescriptionsSubstituteTheRoll()
    {
        ...
        var contribution = AffixGrants.Contributions(rolled, keen, "test").Single();
        Assert.Equal("combat.crit.chance", contribution.Key);
        Assert.Equal(0.07, contribution.Value);

        Assert.Equal("7% critical chance.", AffixGrants.Describe(rolled, keen));
    }
    ...
    public void RuleGrantsSubstituteTheRollIntoChance()
    {
        ...
        var rule = AffixGrants.Rules(rolled, serrated).Single();
